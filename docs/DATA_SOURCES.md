...
The Expectations Gap page includes company-issued guidance from the same primary earnings sources used for Company Capture. That is operating context, not analyst consensus or a valuation input. Financial Modeling Prep is the selected first provider for a compact current snapshot of annual consensus, current price inputs, and price-target context. Its static-site workflow remains blocked until the required public-display agreement is complete, the `FMP_API_KEY` Actions secret exists, and the `FMP_PUBLIC_DISPLAY_APPROVED=true` Actions variable explicitly opens the gate.

When enabled, `scripts/ingest-fmp-market-data.mjs` requests annual analyst estimates, quote data, and price-target consensus for the current Expectations Gap tickers. It creates only `data/processed/fmp-market-expectations.json` in the Pages build workspace: a compact current normalized snapshot with retrieval date, fiscal-period label, and clearly marked mechanical calculations. The file is served by the approved Pages deployment but is ignored by Git, so raw FMP responses, normalized FMP data, and a historical FMP archive are not committed to the public repository. See [FMP public-display setup](FMP_SETUP.md) before activating it.
