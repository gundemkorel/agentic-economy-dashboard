...
Financial Modeling Prep (FMP) is a gated fallback for the Gap Readiness page's compact market-context snapshot. It is intentionally inactive until the account and written rights cover the exact public use below.
...
4. Open **Actions → Deploy dashboard to GitHub Pages → Run workflow**. The deployment requests only the current Gap Readiness tickers: AKAM, ESTC, DT, and FFIV.
