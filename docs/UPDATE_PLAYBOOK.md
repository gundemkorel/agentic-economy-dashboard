...
6. Review the Akamai scenario lab inputs after earnings. Keep company-reported values, attributed current market context, and project assumptions in separate fields; do not backfill vendor market data into the repository.
   ...
   - Update the AKAM nominal cash-recovery screen only when a primary filing changes the conditional commitment or estimated capex; keep retention rates and extra-capex stresses explicitly hypothetical, and rerun the deal-hurdle validator.
...
- Economic Map schema and materiality inputs validated when changed
